...
void RenderCube(Shader& lightCubeShader, unsigned int diffuseMap, unsigned int specularMap, unsigned int emissionMap,unsigned int cubeVAO, Shader& lightingShader);
void RenderImGui(ImVec4& objectcolor);
...
int main()
{
	...
	std::string diffusemap = "source/container2.png";
	std::string specularmap = "source/container2_specular.png";
	std::string emissionmap = "source/matrix.jpg";

	unsigned int diffuseMap = loadTexture(diffusemap.c_str());
	unsigned int specularMap = loadTexture(specularmap.c_str());
	unsigned int emissionMap = loadTexture(emissionmap.c_str());

	lightCubeShader.use();
	lightCubeShader.setInt("material.diffuse", 0);
	lightCubeShader.setInt("material.specular", 1);
	lightCubeShader.setInt("material.emission", 2);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//imgui组件
		RenderImGui(objectcolor);
		//渲染物体
		RenderCube(lightCubeShader, diffuseMap, specularMap, emissionMap, cubeVAO, lightingShader);
...
{
	{
		...
		lightCubeShader.setFloat("material.shininess", 64.0f);

		lightCubeShader.setFloat("matrixlight", (1.0 + sin(glfwGetTime())) / 2 + 0.5);
		lightCubeShader.setFloat("matrixmove", glfwGetTime());

		lightCubeShader.setVec3("light.ambient", 0.2f, 0.2f, 0.2f);
		lightCubeShader.setVec3("light.diffuse", 0.5f, 0.5f, 0.5f); // 将光照调暗了一些以搭配场景
